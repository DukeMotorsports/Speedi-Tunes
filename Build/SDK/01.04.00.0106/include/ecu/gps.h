 ...
 * @details <p>This value is represented as seconds since midnight.</p>
 * @details <p>This value will equal zero (0.0) if no time has been reported.</p>
 * @details <p>Common to both GGA and RMC sentences.</p>
 */
 ...
 * @details </table>
 * @details <p>Contained in GGA sentence only.</p>
 */
 ...
 * @details warning (V).</p>
 * @details <p>Contained in RMC sentence only.</p>
 */
 ...
 * @details <p>Contained in RMC and VTG sentences.</p>
 */
 ...
 * @details <p>This value is represented as DDMMYY, for example 231121 is 23rd November 2021.</p>
 ...
 * @details reported.</p>
 * @details <p>Contained in RMC sentence. Calculated from VTG sentence.</p>
 */
 ...
 * @details <p>Contained in RMC and VTG version 2.3+ sentence only.</p>
 * @details <p>The mapping from character to value are defined in the following table:</p>
 * @details <table class=param>
 * @details <tr><th>FAA Mode</th><th>Fix Quality (Return Value)</th><th>Meaning</th></tr>
 * @details <tr><td></td><td>-1</td><td>no mode reported</td></tr>
 * @details <tr><td>N</td><td>0</td><td>Data Not Valid</td></tr>
 * @details <tr><td>A</td><td>1</td><td>Autonomous mode</td></tr>
 * @details <tr><td>D</td><td>2</td><td>Differential Mode</td></tr>
 * @details <tr><td>R</td><td>4</td><td>RTK Integer mode</td></tr>
 * @details <tr><td>F</td><td>5</td><td>RTK Float mode</td></tr>
 * @details <tr><td>E</td><td>6</td><td>Estimated (dead-reckoning) mode</td></tr>
 * @details <tr><td>M</td><td>7</td><td>Manual Input Mode</td></tr>
 * @details <tr><td>S</td><td>8</td><td>Simulated Mode</td></tr>
 * @details <tr><td>C</td><td>9</td><td>Quectel Querk, "Caution"</td></tr>
 * @details <tr><td>P</td><td>10</td><td>Precise (4.00 and later)</td></tr>
 * @details <tr><td>U</td><td>11</td><td>Quectel Querk, "Unsafe"</td></tr>
 * @details </table>
 */
...
/**
 * @fn bool gps_timeout_gga(f32 time)
 * @arg ScriptName TimeoutGPGGA
 *
 * @brief Detect GGA sentence timeout
 *
 * @details <p>True if no GGA sentences received for
 * @details <span class="param"/>time</span> seconds.</p>
 *
 * @param time Timeout in seconds (minimum 0.1sec).
 */

static __inline __warn_unused_result bool gps_timeout_gga(f32 time)
{
	if (gps.gga_ticks == 0)
		return true;
...
/**
 * @fn bool gps_timeout_rmc(f32 time)
 * @arg ScriptName TimeoutGPRMC
 *
 * @brief Detect RMC sentence timeout
 *
 * @details <p>True if no RMC sentences received for
 * @details <span class="param"/>time</span> seconds.</p>
 *
 * @param time Timeout in seconds (minimum 0.1sec).
 */

static __inline __warn_unused_result bool gps_timeout_rmc(f32 time)
{
	if (gps.rmc_ticks == 0)
		return true;
...
/**
 * @fn bool gps_timeout_vtg(f32 time)
 * @arg ScriptName TimeoutVTG
 *
 * @brief Detect VTG sentence timeout
 *
 * @details <p>True if no VTG sentences received for
 * @details <span class="param"/>time</span> seconds.</p>
 *
 * @param time Timeout in seconds (minimum 0.1sec).
 */

static __inline __warn_unused_result bool gps_timeout_vtg(f32 time)
{
	if (gps.vtg_ticks == 0)
		return true;

	s32 elapsed = ticks_since_u32(gps.vtg_ticks);

	time = (time < 0.1f) ? 2.0f : time; /* 2sec if bogus value */
	if (elapsed < (time / tick_period_f32()))
		return false;

	gps.vtg_ticks = 0; /* timeout */
	return true;
}

/**
 * @fn bool gps_timeout_speed(f32 time)
 * @arg ScriptName TimeoutSpeed
 *
 * @brief Detect GPS speed (and heading) timeout
 *
 * @details <p>True if both RMC and VTG NMEA sentences are not received for
 * @details <span class="param"/>time</span> seconds.</p>
 * @details <p>When False: Speed, MagneticVariation, TrueCourse and optionally Mode are valid.</p>
 *
 * @param time Timeout in seconds (minimum 0.1sec).
 */

static __inline __warn_unused_result bool gps_timeout_speed(f32 time)
{
	return (gps_timeout_rmc(time) && gps_timeout_vtg(time));
}
 ...
 *
 * @details <p>True if either GGA or RMC GPS sentences are not received for
 * @details <span class="param"/>time</span> seconds.</p>
 *
 * @param time Timeout in seconds (minimum 0.1sec).
 */
...
{
	return (gps_timeout_gga(time) || gps_timeout_rmc(time));
}
